  ...
  try {
    ...
    if (!config.baseRequest && !config.request && adapter?.reqClass) {
      ...
      data.baseRequest = async function (url: string, options?: { method?: string; [key: string]: any }) {
        ...
        const requestType = options.method?.toLocaleLowerCase() || 'get'

        const func = adapterReq?.[requestType]
        ...
        try {
          // 兼容非小程序的adapter
          if ((!options.headers['content-type'] || options.headers['content-type'].includes('application/json')) && typeof options.body !== 'string') {
